...
const Ranking = ({ owners, tops }) => {
    ...
    const topHolders = [
        {
            rank: 1,
            address: tops?.[0]?.owner ?? '0x000000000000000000000000',
            quantity: tops?.[0]?.total ?? 0,
            icon: Trophy,
        ...
        {
            rank: 2,
            address: tops?.[1]?.owner ?? '0x000000000000000000000000',
            quantity: tops?.[1]?.total ?? 0,
            icon: Medal,
        ...
        {
            rank: 3,
            address: tops?.[2]?.owner ?? '0x000000000000000000000000',
            quantity: tops?.[2]?.total ?? 0,
            icon: Award,
                                ...
                                <table className="w-full">
                                    ...
                                    {owners.map((holder, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-4">
                                                <span className="font-mono text-white/80 text-sm">{holder.owner.slice(0, 10)}...{holder.owner.slice(-8)}</span>
                                            </td>
                                            <td className="p-4 text-center">
                                                <span className="font-bold text-white">{holder.total}</span>
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
